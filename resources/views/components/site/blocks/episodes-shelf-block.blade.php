...
<section class="{{ $layout['shell_class'] }}" style="{{ $colors['shell_style'] }} {{ $layout['shell_style'] }}">
    <div class="{{ $layout['inner_class'] }} px-4 sm:px-6 lg:px-8 space-y-10" style="{{ $layout['inner_style'] }}">
        {{-- Video Player & Program Info Section --}}
        <div class="grid gap-8 lg:grid-cols-3 items-start">
            {{-- SOL: Video/Player Alanı ve Altında Sadece Kategoriler --}}
            <div class="lg:col-span-2 space-y-4">
                <div class="aspect-video w-full overflow-hidden rounded-2xl bg-black ring-1 ring-white/10 shadow-2xl">
                    @if ($initialSrc)
                ...
                @if ($program->categories && $program->categories->isNotEmpty())
        ...
        @if ($episodes->isNotEmpty())
            ...
            @endphp
            <div x-data="{ limit: {{ $initialLimit }}, total: {{ $episodes->count() }} }" class="space-y-6">
                ...
                <div x-show="limit < total" class="pt-6 text-center">
                    <button type="button"
                            @click="limit += 24"
